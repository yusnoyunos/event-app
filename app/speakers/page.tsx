...
export default function SpeakersPage() {
  return (
    <section className="mx-auto max-w-6xl px-4 pb-20 pt-8 sm:px-6 sm:pt-10 md:px-10 lg:px-12 lg:pt-12">
      ...
      <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:mt-12 lg:grid-cols-4">
        {speakers.map((speaker) => (
          <article
            key={speaker.name}
            className="group flex flex-col justify-between rounded-2xl border border-slate-800/80 bg-slate-950/80 p-5 text-sm text-slate-200 shadow-[0_22px_70px_rgba(15,23,42,0.9)] transition-transform hover:-translate-y-1 hover:border-sky-400/60 hover:shadow-[0_26px_90px_rgba(56,189,248,0.35)]"
          >
            <div className="space-y-3">
              <div className="inline-flex items-center gap-2 rounded-full border border-sky-500/30 bg-slate-900/80 px-3 py-1 text-[11px] font-medium text-sky-200">
            ...
            <div className="mt-4 h-px bg-gradient-to-r from-transparent via-sky-500/50 to-transparent" />
          </article>
        ))}
      </div>
    </section>
